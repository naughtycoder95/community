 ...
 * fileName : SuggestBoardController
 * author : GGG
 * date : 2024-06-05
 * description :
 * 요약 :
...
//            전체 조회 서비스 함수 실행
            Page<ISuggestBoardDto> pageList = suggestBoardService.findAllByBoardTitleContaining(boardTitle, pageable);
...
//               아래 코드는 프론트로(웹브라우저) 신호를(500) 보냄
    ...
    //    자유게시판 공지 전체조회
    @GetMapping("/suggest-notice")
    public ResponseEntity<Object> findNotice(
    ) {
        try {
//            전체 조회 서비스 실행
            List<ISuggestBoardDto> free
                    = suggestBoardService.findByCodeAndNotice();
...
//                데이터 없음
    ...
    //    TODO: 상세조회
    @GetMapping("/suggest/{boardId}")
    public ResponseEntity<Object> findById(
            @PathVariable long boardId
    ) {
        try {
            Optional<Board> optionalBoard = suggestBoardService.findById(boardId);
...
//                데이터 있음(200)
    ...
    public ResponseEntity<Object> create(
        ...
        try {
            suggestBoardService.save(request.getBoard(), request.getVoteDtos(), request.getPlace(), request.getFileDtos(), request.getBoardFileDtos());
...
//            500 전송
    ...
    //    TODO: 수정함수
    @PutMapping("/suggest/{boardId}")
    public ResponseEntity<Object> update(
            @PathVariable long boardId,
            @RequestBody Board board
    ) {
        try {
            suggestBoardService.update(board);
...
//            DB 에러 (서버 에러) -> 500 신호(INTERNAL_SERVER_ERROR)
    ...
    //     TODO: 삭제 함수
    @DeleteMapping("/suggest/deletion/{boardId}")
    public ResponseEntity<Object> delete(
            @PathVariable long boardId
    ) {
        try {
            boolean success = suggestBoardService.removeById(boardId);
