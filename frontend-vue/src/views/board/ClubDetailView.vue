<template>
  <div class="container mt-3 board-detail-container">
    ...
    <div class="row mb-3 board-button d-flex justify-content-end me-2">
      ...
      <button v-if="auth === 'A' || auth === 'B'" class="col-1 btn btn-danger" @click="confirmBoDelete">삭제</button>
    </div>
    <!-- 게시글 -->
    <div class="card mb-3">
      ...
      <div class="card-body">
        ...
        <div class="card mb-3" style="width: 25rem" v-if="vote">
          ...
          <div class="card-body">
            <div v-for="(data, index) in vote" :key="index" :class="{ 'highlight': data.voteId === voteMember.voteId }">
              <div class="form-check-vote d-flex justify-content-between align-items-center">
                <input v-if="!voteMember" class="form-check-input input-vote" type="radio" name="voteRadios"
                  id="voteRadios" :value="data.voteId" />
            ...
            <div class="d-md-flex justify-content-md-end">
              ...
              <button v-else-if="!voteMember" type="button" class="btn btn-secondary" @click="saveVote()">투표하기</button>
            </div>
          </div>
        </div>
        <!-- 장소 -->
        ...
        <div class="d-flex justify-content-start">
          ...
          <!-- 신고 아이콘 -->
          <div v-if="auth === 'C'" class="me-3" type="button" data-bs-toggle="modal" data-bs-target="#reportModal"><i
        ...
        <li v-if="!data.isEditing" class="list-group-item">
          ...
          <div class="d-flex justify-content-between mt-2">
            ...
            <div>
              ...
              <button v-if="this.member.memberId === data.memberId || auth === 'A'" class="btn btn-danger me-3"
                @click="deleteReply(data)">삭제</button>
              <button v-if="this.member.memberId !== data.memberId && auth !== 'A'" class="btn btn-secondary"
                data-bs-toggle="modal" data-bs-target="#reportReplyModal" @click="openReplyReport(data)">
          ...
          <div v-if="showWriteReReply && parentId === data.replyId" class="card mb-3 mt-3">
            <div class="card-body">
              ...
              <div class="d-flex justify-content-between">
                <!-- 파일첨부 -->
                <input class="form-control file-upload-input w-auto" type="file" @change="selectReReplyFile($event)" />
                <button class="btn btn-primary file-upload-button" @click="createReReply(data.replyId)">등록</button>
              </div>
            </div>
          </div>
        </li>
          ...
          <form class="was-validated">
            ...
            <div class="modal-body">
              ...
              <div class="mb-2 row">
                <label for="staticTitle" class="col-sm-2 col-form-label">내용</label>
                <div class="col-sm-10">
                  <input type="text" readonly class="form-control-plaintext" id="staticTitle" v-model="report.reply">
                </div>
              </div>
              <div class="mb-2">
                <hr />
    ...
    <div class="card mb-3">
      <div class="card-body">
        ...
        <div class="d-flex justify-content-between">
          <!-- 파일첨부 -->
          <input class="form-control file-upload-input w-auto" type="file" ref="replyFile" @change="selectReplyFile" />
...
export default {
  ...
  methods: {
    ...
    // 글 수정 페이지로 이동
    moveToEdit() {
      this.$router.push(`/board/club-edit/${this.bocode}/${this.smcode}/${this.boardId}`);
    },
    // 추천 버튼 클릭 시 호출 
    toggleRecommend() {
