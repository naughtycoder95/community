<template>
    <div class="container mt-3 board-detail-container">
        ...
        <div class="reply-content mb-3">
            <ul v-for="(data, index) in reply" :key="index" class="list-group mb-3">
                ...
                <li v-if="!data.isEditing" class="list-group-item">
                    <div class="reply-name">{{ data.nickname }}</div>
                ...
                <div v-for="(reReply, index) in data.reReplies" :key="index">
                    <li v-if="!reReply.isReReplyEditing" class="list-group-item reReply-container">
                        <div class="reply-name"><i class="bi bi-arrow-return-right"></i> {{ reReply.nickname }} </div>
        ...
        <!-- 목록으로 돌아가기 버튼 -->
        <div class="d-grid">
            <button class="col-1 btn btn-secondary" @click="this.$router.push(`/board/praise`)">목록</button>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        // 글 수정 페이지로 이동
        moveToEdit() {
            this.$router.push(`/board/praise-edit/${this.boardId}`);
        },
        // 추천 버튼 클릭 시 호출 
        toggleRecommend() {
